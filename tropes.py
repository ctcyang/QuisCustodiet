...
class Tropes(object):
  ...
  # Take source code (page) and insert it into DB 
  def add_page(self, url, page):
    #TODO Purge won't work
    self.c.execute("INSERT INTO pagecache VALUES (?, ?)", (url, page))
    self.sql.commit()

  # Read a list of urls from "filteredworks.txt", compare with those in DB
  # If not in DB, download and insert into DB
  def cache_pages(self, url_file):
    with open(url_file) as src:
      urls = src.readlines()
      # Fetch existing URLs in DB and put them in hash table
      # Do not redownload these URLs
  ...
  # Function is used if constraints were changed.
  ...
  # Take the edges as generated by list edges and calculate the pagerank of the nodes
  ...
  # Save shoutouts in the form of an edge "work -> ref" in DB
